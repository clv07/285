...
class AMDMTrainer(trainer_base.BaseTrainer):
    ...
    def __init__(self, config, dataset, device):
        ...
        self.full_T = optimizer_config.get('full_T', False)  # kept for compatibility, but not needed anymore
    ...
    def compute_teacher_loss(self, model, sampled_frames, extra_info, ep):
        # last_frame = sampled_frames[:,0,:]
        # ground_truth = sampled_frames[:,1,:]

        # use previous frame to predict next frame in the window N, and calculate average loss
        N = self.window_size
        
        # generate cold start frames
        # print(f'Sampled frames = {sampled_frames.shape}') # torch.Size([4096, optimizer.rollout, 267])
        if ep == 0 and self.cold_start is not None:
            with torch.no_grad():
                window_frames = self.cold_start.eval_seq(sampled_frames[:,0,:], extra_dict=None, num_steps=N+1, num_trials=1)
            # print(f'cold_start={window_frames.shape}') # cold_start=torch.Size([4096, 1, N+1, 267])
            sampled_frames = window_frames[:,0,:,:] 
            # print(f'sampled_frames after cold start = {sampled_frames.shape}')
            self.cold_start = None
            
        B, T, D = sampled_frames.shape
        self.optimizer.zero_grad()        
        start = T - (N+1)
        loss = 0.0
        total_loss = 0.0
        ground_truth = sampled_frames[:,N,:]
        
        for i in range(start, start + N):
            # print(f'In the loop: i={i}')
            last_frame = sampled_frames[:,i,:]
            # print(f'last_frame shape={last_frame.shape}, ground_truth shape={ground_truth.shape}')
            diff_loss, pred_frame = model.compute_loss(last_frame, ground_truth, None, extra_info)
            loss += self.diffusion_loss_weight * diff_loss
            total_loss += loss / N #moving average
        ...
        # return {"diff_loss":loss.item()}
        return {"diff_loss":total_loss.item()}
    ...
    def compute_student_loss(self, model, sampled_frames, sch_samp_prob, extra_info):
        ...
        for st_index in range(self.num_rollout -1):
            ...
            else: # optimizer full_T is false
                if st_index == 0:
                    last_frame = sampled_frames[:,0,:]
                else:
    ...
    def train_loop(self, ep, model):
        ...
        for frames in pbar:
            ...
            loss_dict= self.compute_teacher_loss(model, frames, extra_info=extra_info, ep=ep)
